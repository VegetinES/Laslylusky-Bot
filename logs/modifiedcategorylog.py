...
class ModifiedCategoryLogs(commands.Cog):
    ...
    async def log_category_update_event(self, before, after):
        try:
            if not after.guild:
                return
                
            guild_data = get_specific_field(after.guild.id, "audit_logs")
            if not guild_data or "mod_cat" not in guild_data:
                return
            
            mod_category_config = guild_data["mod_cat"]
            
            is_activated = mod_category_config.get("activated")
            ...
            log_channel_id = mod_category_config.get("log_channel")
            ...
            track_name = mod_category_config.get("changedname", False)
            track_perms = mod_category_config.get("changedperms", False)
            
            if not ((track_name and name_changed) or (track_perms and perms_changed)):
                return
            
            message_data = mod_category_config.get("message", {})
            message_format = mod_category_config.get("mod_cat_messages", "")
            
            replacements = {
                "{category}": after.name,
                "{categoryid}": str(after.id)
            }
            ...
            if message_data and isinstance(message_data, dict):
                ...
                embed = discord.Embed(
                    ...
                    description=f"**Categoría:** {after.name} ({after.id})",
            ...
            print(f"Error en log_category_update_event: {e}")

    @commands.Cog.listener()
    async def on_guild_channel_update(self, before, after):
        if isinstance(after, discord.CategoryChannel):
            await self.log_category_update_event(before, after)

async def setup(bot):
    await bot.add_cog(ModifiedCategoryLogs(bot))